...
for leftDist in [
    ...
    for rightDist in ['uniform_2']:
        for agg in ['count', 'sum', 'avg']:
            for cond_type in ['eq', 'geq']:
                for p in prob:
                    synthetic_preset.append(
                        (impala_host, impala_port, 'synthetic_10m',
                         'synthetic_10m_preset_where1', agg, leftDist,
                         rightDist, cond_type, p[0], p[1],
        ...
        for p in prob:
            instacart_preset.append(
                (impala_host, impala_port, 'instacart', 'instacart_preset_new',
                 agg, cond_type, p[0], p[1], num_instacart_samples, False))

# run
results = []
for arg in synthetic_ours:
    results.append(
        pool.apply_async(es.run_synthetic_ours_with_where,
                         arg,
                         callback=callback_success,
                         error_callback=callback_error))
for arg in synthetic_preset:
    results.append(
        pool.apply_async(es.run_synthetic_preset_with_where,
                         arg,
                         callback=callback_success,
                         error_callback=callback_error))
for arg in instacart_ours:
    results.append(
        pool.apply_async(es.run_instacart_ours_with_where,
                         arg,
                         callback=callback_success,
                         error_callback=callback_error))
for arg in instacart_preset:
    results.append(
        pool.apply_async(es.run_instacart_preset_with_where,
                         arg,
                         callback=callback_success,
                         error_callback=callback_error))
pool.close()
